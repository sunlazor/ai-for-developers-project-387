...
The `name` must match a database defined in the `databases` section of the same Blueprint, or an existing database in
the workspace.
...
This adds all variables from the named group to the service. The group must exist in the workspace or be defined in
`envVarGroups`.
...
- **sync: false only prompts on initial Blueprint setup.** On subsequent syncs, existing `sync: false` values are
  preserved. Adding a new `sync: false` var to an existing Blueprint does not prompt.
...
- **fromService/fromDatabase can reference services outside the Blueprint** but the referenced service must already
