...
**Fix:** Follow the setup instructions in the main SKILL.md for your specific tool (Cursor, Claude Code, Codex). After
adding the config, restart the tool.
...
- Added via CLI:
  `claude mcp add --transport http render https://mcp.render.com/mcp --header "Authorization: Bearer <key>"`
...
If MCP cannot be configured (network restrictions, unsupported tool), use the Render CLI as a fallback. See **render-cli
** for setup and usage.
