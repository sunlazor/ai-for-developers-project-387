...
- **External** (local dev, external tools, CI outside Render):
  ...
  TLS **1.2+** is required for external endpoints;
  see [Render database documentation](https://render.com/docs/databases) for current TLS and networking requirements.
...
- **`property: connectionString`** — single URL (ensure the app uses internal vs external appropriately at runtime if
  you override per environment), or
...
3. Point applications at the **same host and port** and **same user/password**, changing only the **path** (database
   name) in the URL:
