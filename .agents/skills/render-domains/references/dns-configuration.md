...
**Preferred approach:** Use a DNS provider that supports CNAME flattening or ALIAS records (Cloudflare, Namecheap,
DNSimple, NS1). This avoids hardcoding an IP that could change.

### www + apex pattern

When you add `www.example.com`, Render automatically adds `example.com` with a redirect to `www`. When you add
`example.com`, Render adds `www.example.com` with a redirect to the root.
...
2. For apex domains: Check if your provider supports ALIAS/ANAME records. If not, use an A record with the IP from the
   Render Dashboard.
...
**Remove all AAAA records** for domains you point to Render. Render uses IPv4 only. Stale AAAA records cause some
clients to attempt IPv6 connections that fail.
