...
Render automatically provisions and renews TLS certificates (via Let's Encrypt and Google Trust Services) for all custom
domains. All HTTP traffic is redirected to HTTPS. Custom domains work on **web services** and **static sites** only.
...
**Important:** Remove any `AAAA` (IPv6) records for your domain. Render uses IPv4, and stale `AAAA` records cause
unexpected behavior.
...
**Cloudflare users:** If you add `*.example.com` without adding the root domain to Render, disable proxying (gray cloud)
for the root domain to avoid routing conflicts.
...
Blueprint `domains` only **declare** the domain association. You still need to configure DNS with your provider
manually.
