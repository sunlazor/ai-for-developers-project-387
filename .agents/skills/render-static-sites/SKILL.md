...
Deploys static frontends (React, Vue, Hugo, Gatsby, Docusaurus, Jekyll, etc.) to Render's global CDN with automatic TLS,
Brotli compression, HTTP/2, and DDoS protection. Free tier available.
...
For SSR frameworks (Next.js, Nuxt, SvelteKit) that need a running server, use **render-web-services** instead. For
Blueprint authoring, see **render-blueprints**.
...
**Key constraint:** Static sites are **not on the private network**. They cannot communicate with other Render services
...
Render auto-detects and installs dependencies. Set `SKIP_INSTALL_DEPS=true` to handle installation yourself in the build
command.
...
Configure **Redirect/Rewrite Rules** in the Dashboard (Settings > Redirects/Rewrites) or via the Blueprint `routes`
field:
