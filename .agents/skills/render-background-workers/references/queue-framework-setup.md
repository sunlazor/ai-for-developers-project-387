...
Minimal patterns for running each stack as a **`type: worker`** service. Replace regions, plans, and commands to match
your repo.
...
Set Key Value **maxmemory policy** to **`noeviction`** in the Dashboard so queue keys are never evicted like cache
entries.
...
Oban uses **PostgreSQL** as its queue store—**not** Redis. Provision a Render **PostgreSQL** database and wire *
*`DATABASE_URL`**.
...
Do **not** point Oban at Key Value unless you are using an **experimental or custom** backend; default Oban is *
*Postgres-only**.
